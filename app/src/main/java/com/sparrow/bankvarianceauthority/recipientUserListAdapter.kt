...
class recipientUserListAdapter(val context : Context, val items : ArrayList<Customer>,val UUID : String):RecyclerView.Adapter<recipientUserListAdapter.ViewHolder>(){
...
//        }
        holder.csl_userTile.setOnClickListener{
            val intent = Intent(it.context,paymentScreen::class.java)
            intent.putExtra("donor",UUID)
            intent.putExtra("recipient",items[position].UID)
            it.context.startActivity(intent)
        }
